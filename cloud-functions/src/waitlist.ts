    ...
    .onDelete(async (snap, context) => {
        ...
        try {
            await db.runTransaction(async (transaction) => {
                ...
                const participantsSnapshot = await transaction.get(eventRef.collection('participants'));
                ...
                const userParticipatingRef = db.collection('users').doc(promotedUserId).collection('participating').doc(eventId);

                transaction.set(newParticipantRef, nextUser);
                transaction.set(userParticipatingRef, { eventId: eventId, joinedAt: new Date().toISOString() });
